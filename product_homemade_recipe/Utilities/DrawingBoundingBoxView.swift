...
class DrawingBoundingBoxView: UIView {
    ...
    public var predictedObjects: [FritzVisionObject] = [] {
        didSet {
    ...
    func drawBoxs(with predictions: [FritzVisionObject]) {
    ...
    func createLabelAndBox(prediction: FritzVisionObject) {
        let labelString: String? = prediction.label
        let color: UIColor = labelColor(with: labelString ?? "N/A")
        
        let scale = CGAffineTransform.identity.scaledBy(x: bounds.width, y: bounds.height)
        let transform = CGAffineTransform(scaleX: 1, y: -1).translatedBy(x: 0, y: -1)
        let bgRect = prediction.boundingBox.cgRect.applying(transform).applying(scale)
